...
cohort %>% select_if(is.character)


plots2= cohort %>% select_if(is.character) %>% map(~ggplot(cohort, aes(hypertensives, ..count..)) + geom_bar(aes(fill = .), position = "dodge")+ labs(title=.))
paths2 <- stringr::str_c("factor",1:length(plots2), ".png")
